...
pacman::p_load(
  ...
  GGally, ggcorrplot, lubridate, dplyr, tidyr, stringr, stringi, purrr, tibble, here, webshot2, gt, osmdata, sf, tidyverse, here, units, progressr,osmextract, rosm, ggspatial, prettymapr, nngeo, RANN, textclean, spatialsample, blockCV, tidymodels, SuperLearner, nnls, recipes, glmnet, xgboost, ranger
)
...
# Fórmula SIN interacciones (recipe las crea)
...
# EXTRAER nombres dummies


rec_basic <- recipe(sl, data = train_learner) %>%
  step_dummy(all_nominal_predictors(), one_hot = TRUE)

prep_basic <- prep(rec_basic)

vars <- names(bake(prep_basic, train_learner))

year_dums  <- grep("^year_", vars, value = TRUE)
ptype_dums <- grep("^property_type_", vars, value = TRUE)
amenities  <- c("has_seguridad","has_balcon","has_piscina","has_gimnasio")


# RECETA FINAL ROBUSTA

rec <- recipe(sl, data = train_learner) %>%
  step_dummy(all_nominal_predictors(), one_hot = TRUE) %>%

  # Interacciones pandemia
  step_interact(
    terms = as.formula(
  ...
  # Interacciones propiedad × amenidades
  step_interact(
    terms = as.formula(
...
X_train <- bake(prep_rec, train_apto) %>% select(-price)
y_train <- train_apto$price
X_test  <- bake(prep_rec, test_apto)


#  SuperLearner — MODELOS ESTABLES


mix <- c(
  "SL.lm",
  "SL.ranger",
  "SL.mean"        # muy estable, evita explosiones
)
...
# Predicción consistente, sin negativos
test_prediccion <- pmax(test_prediccion, 1)


# Construir archivo Kaggle


sl_ranger_lm_02 <- test_apto %>%
  select(property_id) %>%
  mutate(price = test_prediccion)
