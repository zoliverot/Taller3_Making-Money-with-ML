...
# ============================================================
# 1. FILTRAR SOLO APARTAMENTOS
# ============================================================

train_apto <- train_final %>% 
  filter(property_type == "Apartamento")

test_apto <- test_final   # Kaggle test → 94% apartamentos

# ============================================================
# 2. VARIABLES FACTOR (ROBUSTO)
# ============================================================
...
# ============================================================
# 3. CONVERTIR AMENIDADES A NUMÉRICO (SOLO LAS EXISTENTES)
# ============================================================
...
# ============================================================
# 5. RECETA ROBUSTA PARA TRAIN/TEST
# ============================================================
...
prep_rec <- prep(rec, training = train_apto, retain = TRUE)

# Matrices finales
X_train <- bake(prep_rec, new_data = train_apto) %>% select(-price)
y_train <- train_apto$price
X_test  <- bake(prep_rec, new_data = test_apto)

# ============================================================
# 6. SUPERLEARNER ESTABLE
# ============================================================

mix <- c(
  "SL.lm",      # lineal estable
  "SL.ranger",  # random forest estable
...
test_pred <- predict(sl_fit, newdata = as.data.frame(X_test))$pred

# valores mínimos coherentes
test_pred <- pmax(test_pred, 1)

# ============================================================
# 8. ARCHIVO PARA KAGGLE
# ============================================================

kaggle_output <- test_apto %>%
  select(property_id) %>%
  mutate(price = test_pred)

write.csv(
  kaggle_output,
  here("outputs", "kaggle_submissions", "sl_ranger_lm_03.csv"),
  row.names = FALSE
)

saveRDS(sl_fit, here("outputs", "kaggle_submissions", "sl_ranger_lm_03.rds"))
saveRDS(kaggle_output, here("outputs", "kaggle_submissions", "submission_data.rds"))
