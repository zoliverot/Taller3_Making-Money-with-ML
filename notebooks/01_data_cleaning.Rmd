...
rec <- recipe(~ ., data = full) %>%
  
  # Imputar áreas
  step_impute_bag(surface_total,
                  impute_with = imp_vars(surface_covered, bedrooms, bathrooms, property_type)) %>%
  
  step_impute_bag(surface_covered,
                  impute_with = imp_vars(surface_total, bedrooms, bathrooms, property_type)) %>%
  
  # Imputar habitaciones
  step_impute_bag(bedrooms,
                  impute_with = imp_vars(surface_total, surface_covered, bathrooms, property_type)) %>%
  
  # Imputar baños
  step_impute_bag(bathrooms,
                  impute_with = imp_vars(surface_total, surface_covered, bedrooms, property_type))

# Preparar receta
rec_prep <- prep(rec)

# Aplicar imputaciones
full <- bake(rec_prep, full)
