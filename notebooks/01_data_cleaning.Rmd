...
extract_area <- function(text) {
  ...
  # capturar: "260 m2", "260mts", "260 metros", "260  mts", "260m²"
...
```{r}
registros_total1 <- train_sf_clean1 %>%
  st_drop_geometry() %>%
  filter(surface_total > 1000) %>%
  select(surface_covered, surface_total, description)

registros_total1
```
```{r}
# Valores anómalos de surface_total en TRAIN
train_outliers <- train_sf_clean1 %>%
  st_drop_geometry() %>%
  filter(surface_total > 1200 | surface_total < 20) %>%
  select(surface_total, surface_covered, price, description)

train_outliers

```
```{r}
test_outliers <- test_ready_osm_estrato1 %>%
  st_drop_geometry() %>%
  filter(surface_total > 1200 | surface_total < 20) %>%
  select(surface_total, surface_covered, description)
...
plot_surface_total_zoom(train_final, "TRAIN")
