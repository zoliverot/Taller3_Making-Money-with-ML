...
```{r}
#Base de datos test
test <- read_csv("C:/Users/laura/OneDrive/Documentos/MAESTRIA EN ECONOMIA APLICADA/2025-02/BIG DATA Y MACHINE LEARNING/Taller 3/Taller3_Making-Money-with-ML/data/test.csv")
##Base de datos train
train <- read_csv("C:/Users/laura/OneDrive/Documentos/MAESTRIA EN ECONOMIA APLICADA/2025-02/BIG DATA Y MACHINE LEARNING/Taller 3/Taller3_Making-Money-with-ML/data/train.csv")
```
...
```{r}
# Función para evaluar variables binarias vs. precio
...
#  Ranking y selección de las mejores
...
# Cargar fuentes externas 
  ...
  if (!file.exists(ruta_gpkg)) stop("No se encontró el archivo 'LOTE.gpkg' en: ", ruta_gpkg)
  if (!file.exists(ruta_estrato_csv)) stop("No se encontró el archivo 'estrato.csv' en: ", ruta_estrato_csv)
...
extract_count <- function(text, num_regex, word_regex, base_words) {
  ...
  # =====================================================
  # 0) DECIMALES reales tipo "1.5 baños"
  # =====================================================
  ...
  # =====================================================
  # 1) Interpretar "35 baños" como 3.5 → 4
  # =====================================================
  ...
  if (!is.na(two_digit[1])) {
    ...
    d2 <- num %% 10
    val <- d1 + d2/10    # 35 → 3.5
    return(round(val))   # redondear a entero
  }
  ...
  # =====================================================
  # 3) "1 baño y medio" → 1.5 → 2
  # =====================================================
  half_num <- str_match(txt, "(\\d+)\\s*(baños|banos|baño|bano|bao|baos|wc)\\s*y\\s*medio")
  if (!is.na(half_num[1])) {
    val <- as.numeric(half_num[2]) + 0.5
    return(round(val))
  }

  # "dos baños y medio" → 2.5 → 3
  half_word <- str_match(txt,
    "(uno|una|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)\\s*(baños|banos|baño|bano|bao|baos|wc)\\s*y\\s*medio"
  )
  if (!is.na(half_word[1])) {
    val <- word_to_num(half_word[2]) + 0.5
    return(round(val))
  }

  # "medio baño" → 0.5 → 1
  if (str_detect(txt, "medio\\s*(baño|banos|baños|wc)"))
    return(1)

  # =====================================================
  # 4) Números normales ("3 baños", "2 cuartos")
  # =====================================================
  ...
  vals <- vals[vals >= 1 & vals <= 12]  # rango razonable

  if (length(vals) > 0) return(max(vals))

  # =====================================================
  # 6) Solo aparece la palabra → asignar 1
  # =====================================================
...
extract_from_text <- function(text) {
  ...
  baños <- extract_count(
    ...
    word_regex = "(uno|una|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)\\s*(baños|banos|baño|bano|bao|baos|wc)",
    base_words = c("baños","banos","baño","bano","bao","baos","wc")
  )

  habs <- extract_count(
    text,
    num_regex  = "(\\d+)\\s*(habitaciones|habitacion|cuartos|cuarto|alcobas|alcoba)",
    word_regex = "(uno|una|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)\\s*(habitaciones|habitacion|cuartos|cuarto|alcobas|alcoba)",
    base_words = c("habitaciones","habitacion","cuartos","cuarto","alcobas","alcoba")
  )
...
# ======================================================
# Conservar geometría (sin duplicados)
# ======================================================
...
fill_from_text <- function(df) {
  df %>%
    mutate(
      ...
      bedrooms = ifelse(is.na(bedrooms) & !is.na(text_bedrooms),
                        text_bedrooms, bedrooms),
...
n_test_original  <- nrow(test_ready_osm_estrato1)
...
test_ready_osm_estrato1 <- filtrar_ciudades(test_ready_osm_estrato1, patron_ciudades)

# ================================
# 7. RESUMEN DE ELIMINADOS
# ================================
train_eliminados <- n_train_original - nrow(train_sf_clean1)
test_eliminados  <- n_test_original - nrow(test_ready_osm_estrato1)

cat("🔍 **Resumen de registros eliminados**\n")
cat("Train eliminados:", train_eliminados, "\n")
cat("Test eliminados:",  test_eliminados, "\n")
...
registros_banos_altos1 <- train_sf_clean1 %>%
  st_drop_geometry() %>%
  filter(bathrooms > 10) %>%
  select(bathrooms, bedrooms, surface_total, description)
...
train_sf_clean1 %>%
  ...
  geom_histogram(bins = 30, fill = "#1f78b4", color = "white", alpha = 0.8) +
  labs(
    title = "Distribución de cuartos — TRAIN",
    x = "Número de baños",
    y = "Frecuencia"
  ) +
  theme_minimal(base_size = 14)
...
casos_graves <- train_sf_clean1 %>%
  ...
  select(price, surface_total, bedrooms, bathrooms, description)
...
REVISAR QUE HACER CON LOS OUTLIERS, SI QUITARLOS ANTES DE LOS MODELOS????
####### HASTA AQUI PUNTO DE CORTE REVISION LAURA D #####
...
# =======================================================
# 1) FIX LÓGICO surface_total ↔ surface_covered
# =======================================================

mutual_surface_fix <- function(df) {
  df %>%
    mutate(
...
train_sf_clean2 <- mutual_surface_fix(train_sf_clean1)
test_ready_osm_estrato2 <- mutual_surface_fix(test_ready_osm_estrato1)


# =======================================================
# 2) Preparación de dataset TABULAR para imputar
# =======================================================

train_imp <- train_sf_clean2 %>%
  st_drop_geometry() %>%
  select(surface_total, surface_covered, bedrooms, bathrooms, property_type)

test_imp <- test_ready_osm_estrato2 %>%
  st_drop_geometry() %>%
  select(surface_total, surface_covered, bedrooms, bathrooms, property_type)

# Combinar para imputar de forma conjunta
full <- bind_rows(
  train_imp %>% mutate(dataset = "train"),
  test_imp  %>% mutate(dataset = "test")
)

# Factor necesario para Random Forest
full$property_type <- as.factor(full$property_type)


# =======================================================
# 3) Función general de imputación (Random Forest)
# =======================================================
...
# =======================================================
# 4) IMPUTACIÓN EN ORDEN ÓPTIMO
# =======================================================

# Primero áreas
full <- impute_var(full, "surface_total",
                   predictors = c("surface_covered", "bedrooms", "bathrooms", "property_type"))

full <- impute_var(full, "surface_covered",
                   predictors = c("surface_total", "bedrooms", "bathrooms", "property_type"))

# Luego habitaciones
full <- impute_var(full, "bedrooms",
                   predictors = c("surface_total", "surface_covered", "bathrooms", "property_type"))

# Finalmente baños
full <- impute_var(full, "bathrooms",
                   predictors = c("surface_total", "surface_covered", "bedrooms", "property_type"))
...
train_imputed <- full %>% filter(dataset == "train") %>% select(-dataset)
test_imputed  <- full %>% filter(dataset == "test")  %>% select(-dataset)


# =======================================================
# 6) Insertar imputaciones en las bases sf originales
# =======================================================

train_final0 <- train_sf_clean2 %>%
  mutate(
...
test_final0 <- test_ready_osm_estrato2 %>%
  mutate(
...
# =======================================================
# 7) Validación de inconsistencias (muy importante)
# =======================================================

# Cubierta no puede ser mayor que total
inconsistencias_train <- train_final0 %>%
  filter(surface_covered > surface_total) %>%
  select(surface_total, surface_covered)

inconsistencias_test <- test_final0 %>%
  filter(surface_covered > surface_total) %>%
  select(surface_total, surface_covered)

cat("\nInconsistencias en TRAIN:", nrow(inconsistencias_train), "\n")
cat("Inconsistencias en TEST:", nrow(inconsistencias_test), "\n")

# Mostrar algunos casos si existen:
head(inconsistencias_train)
head(inconsistencias_test)

# =======================================================
# 8) REPARAR INCONSISTENCIAS DESPUÉS DE IMPUTAR
# =======================================================

fix_inconsistencias <- function(df) {
  df %>%
    mutate(surface_total = pmax(surface_total, surface_covered))
}

train_final0 <- fix_inconsistencias(train_final0)
test_final0  <- fix_inconsistencias(test_final0)

sum(train_final0$surface_covered > train_final0$surface_total)
sum(test_final0$surface_covered > test_final0$surface_total)
...
```{r}
# =======================================================
# 📊 GRAFICOS DE CALIDAD DE DATOS (NO ALTERA NADA)
# =======================================================
...
# =============================
# 1) HEATMAP DE FALTANTES
# =============================

plot_missing_heatmap <- function(df, name) {
  df_na <- df %>% 
    st_drop_geometry() %>%
  ...
  if (nrow(df_na) == 0) {
    cat(paste0("✔ No hay faltantes en ", name, "\n"))
    return(NULL)
  }
  ...
  ggplot(df_na, aes(x = reorder(variable, pct_na), y = pct_na)) +
    ...
    labs(title = paste0("Heatmap de Faltantes — ", name),
         x = "Variable", y = "Porcentaje NA") +
    theme_minimal()
}

plot_missing_heatmap(train_final0, "TRAIN")
plot_missing_heatmap(test_final0, "TEST")


# =============================
# 2) SCATTER surface_total vs surface_covered
# =============================

plot_surface_relation <- function(df, name){
  ggplot(df %>% st_drop_geometry(),
         aes(x = surface_total, y = surface_covered)) +
    geom_point(alpha = 0.2, color = "#E57373") +
    geom_abline(slope = 1, intercept = 0, color = "blue", lwd = 1) +
    labs(
      title = paste0("Relación surface_total vs surface_covered — ", name),
      x = "surface_total",
      y = "surface_covered"
    ) +
    theme_minimal()
...
plot_surface_relation(train_final0, "TRAIN")
plot_surface_relation(test_final0, "TEST")


# =============================
# 3) HISTOGRAMAS DE ÁREAS
# =============================

plot_area_hist <- function(df, name){
  df2 <- df %>% st_drop_geometry()

  g1 <- ggplot(df2, aes(x = surface_total)) +
    geom_histogram(bins = 40, fill = "#42A5F5", alpha = 0.8) +
    labs(title = paste("Histograma surface_total —", name),
         x = "surface_total", y = "Frecuencia") +
    theme_minimal()

  g2 <- ggplot(df2, aes(x = surface_covered)) +
    geom_histogram(bins = 40, fill = "#66BB6A", alpha = 0.8) +
    labs(title = paste("Histograma surface_covered —", name),
...
plot_area_hist(train_final0, "TRAIN")
plot_area_hist(test_final0, "TEST")


# =============================
# 4) BOXPLOT de habitaciones y baños
# =============================

plot_box_discrete <- function(df, name){
  df2 <- df %>% st_drop_geometry()

  g1 <- ggplot(df2, aes(x = factor(0), y = bedrooms)) +
    geom_boxplot(fill = "#AB47BC") +
    labs(title = paste("Distribución de bedrooms —", name),
         x = "", y = "bedrooms") +
    theme_minimal()

  g2 <- ggplot(df2, aes(x = factor(0), y = bathrooms)) +
    geom_boxplot(fill = "#FFA726") +
    labs(title = paste("Distribución de bathrooms —", name),
         x = "", y = "bathrooms") +
    theme_minimal()
...
plot_box_discrete(train_final0, "TRAIN")
plot_box_discrete(test_final0, "TEST")
...
```{r}
ggplot(train_final %>% st_drop_geometry(),
       aes(y = surface_total)) +
  geom_boxplot(fill = "#FF7043") +
  coord_cartesian(ylim = c(0, quantile(train_final$surface_total, 0.99, na.rm = TRUE))) +
  labs(title = "Boxplot surface_total — TRAIN (recortado)")
...
############### CODIGO VIVI ORIGINAL
```{r}
# PASO INTERMEDIO: surface_total <-> surface_covered
mutual_surface_fix <- function(df) {
  df %>%
    mutate(
      surface_total = ifelse(
...
train_sf_clean <- mutual_surface_fix(train_sf_clean)
test_ready_osm_estrato <- mutual_surface_fix(test_ready_osm_estrato)


#  Preparar dataset para imputación con modelos
...
# Insertar imputaciones en bases sf originales
...
# RECORTE DE EXTREMOS PARA APARTAMENTOS


MAX_APTO_AREA <- 1500  # límite superior razonable

train_final <- train_final %>%
  mutate(
    surface_total = ifelse(
      property_type == "Apartamento" & surface_total > MAX_APTO_AREA,
      MAX_APTO_AREA,
      surface_total
    )
  )
