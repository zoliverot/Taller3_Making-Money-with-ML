...
# Función: Extraer info del texto
...
extract_from_text <- function(text) {
  ...
  # baños
  baños <- str_extract(txt, "(\\d+)\\s*(baños|banos|baño|bano)")
  baños <- as.numeric(str_extract(baños, "\\d+"))
  ...
  # área total
  area <- str_extract(txt, "(\\d+)\\s*(m2|metros|m|mt|mts)")
...
# Extraer variables desde texto (AMBAS BASES)
...
# 3. Unir columnas con sf SIN ROMPER la geometría


train_geom <- st_geometry(train_sf_clean)
test_geom  <- st_geometry(test_ready_osm_estrato)

train_sf_clean <- train_sf_clean %>%
  st_drop_geometry() %>%
  bind_cols(train_text_features) %>%
  st_set_geometry(train_geom)

test_ready_osm_estrato <- test_ready_osm_estrato %>%
  st_drop_geometry() %>%
  bind_cols(test_text_features) %>%
  st_set_geometry(test_geom)
...
fill_from_text <- function(df) {
  ...
  # Si por alguna razón no existen, crearlas
  if (!"text_bathrooms" %in% names(df)) df$text_bathrooms <- NA_real_
  if (!"text_bedrooms"  %in% names(df)) df$text_bedrooms  <- NA_real_
  if (!"text_surface"   %in% names(df)) df$text_surface   <- NA_real_
