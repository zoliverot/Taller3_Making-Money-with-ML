...
## Exploración  y manipulación de datos principales 
...
add_text_dummies <- function(df, dicc = token_diccionario) {
  cat("\n>>> Iniciando función add_text_dummies()\n")
...
points <- extract_tag(points, "leisure")
polys  <- extract_tag(polys,  "leisure")
lines  <- extract_tag(lines,  "leisure")
...
# Función general (igual que antes)
...
for (nm in names(pois_sf)) {
  message("📏 Calculando distancia a ", nm, " ...")
...
# 1. Transformar de CRS métrico (3116) a geográfico (4326)
train_final <- st_transform(train_final, 4326)
test_final  <- st_transform(test_final, 4326)

# 2. Extraer coordenadas lon/lat
train_coords <- st_coordinates(train_final)
test_coords  <- st_coordinates(test_final)

train_final$lon <- train_coords[, "X"]
train_final$lat <- train_coords[, "Y"]

test_final$lon <- test_coords[, "X"]
test_final$lat <- test_coords[, "Y"]

# 3. Convertir a data frame eliminando la geometría
train_final_df <- train_final %>% st_drop_geometry()
test_final_df  <- test_final  %>% st_drop_geometry()
