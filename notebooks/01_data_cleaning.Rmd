...
# Aumentar tiempo máximo de descarga/lectura y tolerar geometrías
...
# Desactivar motor s2 para evitar errores de topología
sf_use_s2(FALSE)
...
# Filtrar a Bogotá (bounding box aproximado)
...
# Función robusta que busca en todas las capas disponibles
...
# Crear lista de puntos de interés combinando todas las capas
pois_list <- list(
...
# Eliminar vacíos (NULL o sin filas)
...
# Transformar todo a proyección métrica (EPSG:3116)
...
#  Unión espacial con barrios y localidades
...
El objetivo de estas transformaciones es capturar cómo la proximidad a ciertos puntos de interés influye en el valor de una vivienda, diferenciando entre efectos positivos y negativos según la distancia.

- Autopistas: vivir justo al lado reduce el valor (ruido, contaminación), pero estar cerca —sin estar expuesto— puede ser positivo por accesibilidad.

- Parques y zonas verdes: mayor valor cuanto más cerca, aunque con rendimientos decrecientes, por eso se usa una transformación logarítmica.

- Colegios y jardines: un punto óptimo entre 100 m y 500 m, balanceando conveniencia y externalidades (ruido, tráfico).

- Centros comerciales: cercanía moderada es deseable, pero el exceso de tráfico y ruido reduce el atractivo si se está demasiado cerca.

- Restaurantes y cafeterías: capturan vitalidad urbana; la cercanía tiende a ser positiva.

- Estaciones de transporte público: cercanía razonable mejora accesibilidad, pero el exceso de proximidad puede implicar congestión.
```{r}
...
procesar_distancias <- function(df) {
  df %>%
    ...
    mutate(
      # Convertir categóricas en factor con orden lógico
