...
mae_sl_espacial <- mean(abs(sl_02$SL.predict - y_train))
mae_sl_espacial
...
fold_indices_random <- split(sample(1:N), rep(1:K, length.out = N))

cv_random <- list(
  V = K,
  validRows = fold_indices_random
)

sl_random <- SuperLearner(
  Y = y_train,
  X = as.data.frame(X_train),
  SL.library = mix,
  method = "method.NNLS",
...
mae_sl_random <- mean(abs(sl_02$SL.predict - y_train))
mae_sl_random


comparativo_sl_02 <- tibble(
  Modelo = c("SuperLearner CV normal", "SuperLearner CV espacial"),
  MAE = c(mae_sl_normal, mae_sl_espacial)
)
