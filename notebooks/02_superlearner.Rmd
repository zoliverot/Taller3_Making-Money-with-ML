...
receta_01 <- recipe(sl_01, data = train_learner) %>%
  step_dummy(all_nominal_predictors(), one_hot = TRUE) %>%
  step_interact(
    terms = as.formula(
...
train_mat <- bake(prep_receta_01, new_data = train_learner)
...
mae_01 <- mean(abs(sl_fit$SL.predict - y_train))
mae_01
```
...
test_03 <- predict(sl_03, newdata = as.data.frame(X_test))$pred
test_04 <- pmax(test_03, 1)

# Acotar para subir a kaggel

output_03 <- test_learner %>%
  mutate(price = round(test_03)) %>%
  select(property_id, price)
...
write.csv(
  output_03,
  here("outputs", "kaggle_submissions", "sl_03.csv"),
  row.names = FALSE
)


mae_sl_03 <- mean(abs(sl_03$SL.predict - y_train))
mae_sl_03
...
#  Selección de variables y reparación final
...
# Entrenamiento y tunning de modelos
# Random forest
...
# Precios reales del train filtrado (sin log)
y_real <- train_learner$price

# Predicción del mejor modelo sobre el train completo
log_pred_train <- predict(best_model, newdata = train_data)

# Volver a escala real
pred_train <- expm1(log_pred_train)

# Calcular MAE global
MAE_best <- Metrics::mae(y_real, pred_train)
...
# TABLA PNG DEL MAE DEL MODELO GANADOR


# Calcular MAE global del modelo ganador
y_real <- train_learner$price
log_pred_train <- predict(best_model, newdata = train_data)
pred_train <- expm1(log_pred_train)
...
tabla_gt <- tabla_mae %>%
  gt() %>%
  tab_header(title = "MAE Global – Modelo Ganador")

# Ruta donde guardas el submission
ruta_destino <- here::here("outputs", "kaggle_submissions")
dir.create(ruta_destino, showWarnings = FALSE, recursive = TRUE)

ruta_png <- file.path(ruta_destino, "MAE_best_model_table.png")

# Guardar como PNG
gtsave(tabla_gt, ruta_png)
